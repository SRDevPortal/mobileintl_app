...
frappe.provide("mobileintl_app.desk");

mobileintl_app.desk.is_mobile_app_admin = function () {
...
mobileintl_app.desk.SYNC_TAB_FIELDS = ["profiles_tab", "sessions_tab", "engagement_tab"];

mobileintl_app.desk.SYNC_TABLE_FIELDS = ["profiles", "sessions", "engagement_items"];

mobileintl_app.desk.apply_doctor_desk_restrictions = function (frm) {
	if (!frm || frm.doctype !== "Mobile App User") return;
	if (mobileintl_app.desk.is_mobile_app_admin()) return;

	frm.$wrapper.addClass("ma-doctor-user-form");

	mobileintl_app.desk.SYNC_TAB_FIELDS.forEach((fieldname) => {
		if (frm.fields_dict[fieldname]) {
			frm.set_df_property(fieldname, "hidden", 1);
		}
	});

	mobileintl_app.desk.SYNC_TABLE_FIELDS.forEach((fieldname) => {
	...
	mobileintl_app.desk.hide_create_actions(frm);
};

mobileintl_app.desk.hide_create_actions = function (frm) {
	if (mobileintl_app.desk.is_mobile_app_admin()) return;
...
mobileintl_app.desk.block_new_patient_route = function (frm) {
	if (!frm?.is_new() || mobileintl_app.desk.is_mobile_app_admin()) return false;
