...
def _resolve_mobile_app_user_name() -> str | None:
	"""Match ``mobileintl_app.api.v1._find_user_name`` lookup from form fields."""
	from mobileintl_app.api.v1 import _find_user_name
...
def upload_profile_image():
	...
	**Body:** ``multipart/form-data`` — field ``file`` (image), plus user lookup fields as form fields.
	...
	Example::
	    ...
	    curl -X POST "http://localhost:8000/api/method/mobileintl_app.api.profile_image.upload_profile_image" \\
...
def set_profile_image_from_url(user_name: str, image_url: str):
	...
	Example::
	    ...
	    POST /api/method/mobileintl_app.api.profile_image.set_profile_image_from_url
